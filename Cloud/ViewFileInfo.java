...
public void readFileInfo(HashMap<String,FileInfo> map){
	try{
		...
		java.util.Collections.sort(list,new FileInfo());
		for(int i=list.size()-1;i>=0;i--){
			FileInfo fi = list.get(i);
			Object row[] = {fi.filename,fi.file_id,fi.sha,fi.refrence_id,fi.start_chunk_id,fi.num_chunk,fi.file_size,fi.status,fi.read+"/"+fi.write};
			dtm.addRow(row);
		}
	}catch(Exception e){
		e.printStackTrace();
