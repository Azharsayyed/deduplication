    ...
    try{
		...
		if(input != null){
			...
			if(request.equals("upload")){
				...
				FileInfo fi = new FileInfo(fname,user+"_"+fname,sha,"none",start_chunk_id,row.length,Long.parseLong(size),TFileStatus.goodcompleted_file,row);
				String refrence = checkRefrence(sha,fi);
				fi.refrence_id = refrence;
				...
				if(refrence.equals("none")){
					...
					FileWriter metadata = new FileWriter("metadata/"+fname+".txt");
					metadata.write(buffer1.toString());
					metadata.close();
				}
...
public String checkRefrence(String sha,FileInfo f){
	String result = "none";
	for(Map.Entry<String,FileInfo> entry : map.entrySet()){
		String key = entry.getKey();
		FileInfo fi = entry.getValue();
